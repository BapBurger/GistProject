...
public class SeatController : MonoBehaviour
{
    ...
    [Tooltip("데드존: 이 값보다 작은 미세한 떨림은 무시합니다. (0.05 -> 0.01로 민감하게 변경)")]
    public float deadZone = 0.01f;

    [Header("2. 물리 기반 튜닝 (반응성)")]
    [Tooltip("반응 속도 (추천: 2 ~ 5)")]
    public float tiltSpeed = 5.0f; // 조금 더 빠르게 반응
    [Tooltip("최대 틸트 각도 제한")]
    ...
    public float bolsterGain = 50.0f; // 30 -> 50 증가
    ...
    void ProcessSimulation()
    {
        ...
        // Debug Log (1초에 한번씩만 찍든가 해야하지만 일단 값 확인을 위해)
        ...
        // ------------------------------------------------------------------
        // 우회전 시(Sway > 0) -> 오른쪽 볼스터(RightTarget)
        // 좌회전 시(Sway < 0) -> 왼쪽 볼스터(LeftTarget)
